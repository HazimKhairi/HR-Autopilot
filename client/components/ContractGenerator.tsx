...
export default function ContractGenerator() {
  ...
  const [overrideSalary, setOverrideSalary] = useState<number | undefined>(
    undefined,
  );
  ...
  const lookupEmployee = async () => {
    ...
    try {
      const response = await fetch(
        `${apiBaseUrl}/api/employee/by-email/${encodeURIComponent(email)}`,
      );
    ...
    try {
      const requestBody = useExisting
        ? {
            ...
            ...(overrideSalary ? { salary: overrideSalary } : {}),
          }
        : { customData: formData };

      const response = await fetch(`${apiBaseUrl}/api/contract/generate`, {
        method: "POST",
    ...
    <div className="card">
      ...
      {useExisting ? (
        <div className="mb-4">
          ...
          {employeeData && (
            <div
              className="mt-3 p-3 bg-green-50 border border-green-200 text-green-800"
              style={{ borderRadius: "5px" }}
            >
              <p className="font-medium">{employeeData.name}</p>
              <p className="text-sm">
                {employeeData.role} • {employeeData.country}
              </p>
            </div>
          )}
            ...
            <div className="mt-3 flex items-end gap-2">
              <div className="flex-1">
                ...
                <input
                  ...
                  placeholder={
                    employeeData ? String(employeeData.salary) : "e.g., 8000"
                  }
                />
              </div>
              <div>
        ...
        <div className="space-y-3 mb-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleFormChange("salary", parseInt(e.target.value))
              }
              className="input-field"
              placeholder="8000"
            />
      ...
      {error && (
        <div
          className="bg-red-50 border border-red-300 p-3 text-red-800 text-sm mb-4"
          style={{ borderRadius: "5px" }}
        >
          ...
          <div className="mt-3 flex gap-2">
            <button
              onClick={async () => {
                ...
                try {
                  const filenameBase =
                    (useExisting && employeeData && employeeData.name) ||
                    formData.name ||
                    "employment-contract";
                  ...
                  const pdfResp = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL}/api/contract/render-pdf`,
                    {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
